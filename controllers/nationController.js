...
const getNations = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ msg: "Error retrieving nations", error: error.message }); // Añade el mensaje de error
  }
};
...
const getMonthlyNation = async (req, res) => {
  try {
    ...
    const nation = await Nation.findOne({ monthlyWinner: true }).select(
      "name _id",
    );
    ...
    res
      .status(500)
      .send({ msg: "Error retrieving monthly nation", error: error.message });
  }
};
...
const getNationCreator = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Error retrieving nation creator",
      error: error.message,
    });
  }
};
...
const getNationsUser = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res
        .status(400)
        .send({ msg: "userId is required in query parameters" });
    }
    ...
    res
      .status(500)
      .send({ msg: "Error retrieving nations", error: error.message });
  }
};
...
const getNationDetails = async (req, res) => {
  try {
    ...
    if (!nationId) {
      return res
        .status(400)
        .send({ msg: "nationId is required in query parameters" });
    }
    ...
    res
      .status(500)
      .send({ msg: "Error retrieving nation details", error: error.message });
  }
};
...
const createNationGemini = async (req, res) => {
  try {
    ...
    console.log(
      `📋 Parámetros: nombre=${req.body.nationName}, gobierno=${req.body.governmentType}, era=${req.body.age}`,
    );
    ...
    if (req.body.advanced == true) {
      ...
      nationString = await generateNationGemini(
        req.body.nationName,
        req.body.governmentType,
        req.body.age,
      );
    ...
    console.log(
      `📜 Contexto histórico: ${nationJSON.historicalContext.substring(0, 50)}...`,
    );
    console.log(
      `🌐 Contexto geopolítico: ${nationJSON.geopoliticalContext.substring(0, 50)}...`,
    );
    console.log(
      `🗳️ Detalles políticos: ${Object.keys(nationJSON.politicsDetails).length} atributos`,
    );
    console.log(
      `💹 Detalles económicos: ${Object.keys(nationJSON.economyDetails).length} atributos`,
    );
    console.log(
      `👥 Detalles demográficos: ${Object.keys(nationJSON.populationDetails).length} atributos`,
    );
    ...
    if (nationJSON.events && nationJSON.events.length > 0) {
      console.log(
        `📅 Corrigiendo formato de fechas en ${nationJSON.events.length} eventos...`,
      );
      nationJSON.events = nationJSON.events.map((event) => {
        ...
        if (event.date && event.date.match(/^-\d+/)) {
          ...
          if (yearMatch) {
            ...
            console.log(
              `📅 Fecha corregida: de ${yearMatch[0]}-01-01 a ${event.date}`,
            );
          }
        }
        return event;
    ...
    if (
      ...
      nationJSON.populationDetails.languages
    ) {
      console.log(
        `🔤 Asegurando campos requeridos en ${nationJSON.populationDetails.languages.length} idiomas...`,
      );
      nationJSON.populationDetails.languages =
        nationJSON.populationDetails.languages.map((lang) => {
          if (!lang.status) {
            lang.status = lang.usage || "Oficial";
            console.log(
              `🔤 Agregado status a idioma ${lang.name || lang.languageName}: ${lang.status}`,
            );
          }
          return lang;
        });
    ...
    if (
      nationJSON.populationDetails &&
      nationJSON.populationDetails.religions
    ) {
      console.log(
        `⛪ Asegurando campos requeridos en ${nationJSON.populationDetails.religions.length} religiones...`,
      );
      nationJSON.populationDetails.religions =
        nationJSON.populationDetails.religions.map((religion) => {
          if (!religion.influence) {
            // Determinar influence en base al porcentaje si está disponible
            const percent = parseFloat(
              religion.percentage || religion.percentageAdherents || "0",
            );
            if (percent > 50) {
            ...
            console.log(
              `⛪ Agregado influence a religión ${religion.name || religion.religionName}: ${religion.influence}`,
            );
          }
          return religion;
        });
    ...
    if (
      nationJSON.populationDetails &&
      !nationJSON.populationDetails.growthRate
    ) {
      nationJSON.populationDetails.growthRate =
        nationJSON.populationDetails.populationGrowthRate || "1.5%";
      console.log(
        `👨‍👩‍👧‍👦 Agregado growthRate: ${nationJSON.populationDetails.growthRate}`,
      );
    }
...
const createRandomNation = async (req, res) => {
  try {
    ...
    console.log(`🏛️ Nombre: ${nationJSON.name}`);
    console.log(
      `🌐 Tipo de gobierno: ${nationJSON.politicsDetails.governmentType || "No especificado"}`,
    );
    console.log(
      `📜 Contexto histórico: ${nationJSON.historicalContext.substring(0, 50)}...`,
    );
    ...
    if (
      nationJSON.populationDetails &&
      !nationJSON.populationDetails.population
    ) {
      console.log(
        `🔄 Adaptando estructura de populationDetails al esquema para nación aleatoria...`,
      );
    ...
    const savedNation = await newNation.save();
    console.log(
      `✨ Nación aleatoria guardada con éxito. ID: ${savedNation._id}`,
    );
    ...
    res
      .status(500)
      .send({ msg: "Error creating random nation", error: error.message });
  }
};
